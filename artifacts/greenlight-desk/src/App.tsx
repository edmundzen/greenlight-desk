...
import { useAnalyzeScreenplay, useDecideScreenplay, useGetScreenplay, useListScreenplays, useRestartScreenplay, getGetScreenplayQueryKey, getListScreenplaysQueryKey } from '@workspace/api-client-react';
...
function KeyArt({ detail }: { detail?: ScreenplayDetail }) {
  return (
    <section className="overflow-hidden rounded-xl border border-border bg-card shadow-[0_8px_24px_rgba(77,61,40,.035)]">
      ...
      <div className="flex items-center justify-between px-5 py-3.5"><span className="text-[11px] text-muted-foreground">{detail?.keyArtUrl ? 'Generated from coverage' : 'Generated after analysis completes'}</span>{detail?.keyArtUrl && <button data-testid="button-open-key-art" onClick={() => window.open(detail.keyArtUrl ?? '', '_blank', 'noopener,noreferrer')} className="flex items-center gap-1 text-[11px] font-semibold text-primary hover:underline">Open <ArrowUpRight size={12} /></button>}</div>
    </section>
  );
}
